...
Tiangou-AI vendors only the minimal Julia files needed for the embedded solver
handoff. The upstream Docker and Makefile helpers are intentionally not copied;
from the Tiangou-AI repo root, use `python -m app.gridsfm_solver check`,
`python -m app.gridsfm_solver instantiate`, and
`python -m app.gridsfm_solver run <manifest>` instead.
