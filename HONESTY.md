...
- **Next steps** — Integrate Julia GridSFM for real OPF (already done for a great part of optimization but needs to be expanded); train PINN on more blackout data (current dataset already in repo); add UFLS (under-frequency load shedding) relay model to simulator; source real generator inertia and ramp-rate data from public utility reports.
