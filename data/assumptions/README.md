...
The PowerModels bundle keeps raw demo exports and solver-sanitized exports separate. Raw demo cases preserve OSM-derived visual detail and may contain passive islands, inferred branch charging, synthetic connectors, and voltage mismatches that make strict AC-OPF infeasible. Solver-sanitized cases may remove or relax those blockers, but every action is written into `solver_sanitized`, `sanitization_actions`, and `solver_sanitization_summary` metadata so the handoff remains provenance-labeled rather than silently edited.
